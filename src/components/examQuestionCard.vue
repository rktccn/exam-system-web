    ...
    >
      <template #header-extra>
        <span v-if='showAnswer'>
          <n-icon :size='25' :color='commonLight.errorColor' v-if='!question.answer.isCorrect'><close-round /></n-icon>
          <n-icon :size='25' :color='commonLight.successColor' v-else><check-round /></n-icon>
        </span>
      ...
      <template v-if='question.question.type <= 1'>
        ...
        >
          ...
          <RUnderline v-if='showAnswer' :type='item.isCorrect ? "success":"error"'>{{ item.content }}</RUnderline>
      ...
      <template v-if='question.question.type === 3'>
        ...
        >
          ...
          <RUnderline v-if='showAnswer' :type='question.options[0].content==="正确"?"success":"error"'>正确</RUnderline>
          <div v-else>正确</div>
        </div>
        ...
        >
          ...
          <RUnderline v-if='showAnswer' :type='question.options[0].content==="错误"?"success":"error"'>错误</RUnderline>
          <div v-else>错误</div>
        </div>
      </template>
      ...
      <template #action v-if=' showAnswer
          '>
        <div v-if='question.question.type === 2 || question.question.type === 4'>
          <p>你的回答：
            <RUnderline :type='question.options[0].content===question.answer.content?"success":"error"'>
      ...
      <template #action v-else>
        ...
        <template v-if='question.question.type === 2|| question.question.type ===4'>
          <n-input
